import Link from "next/link";
import { createAdminClient } from "@/lib/supabase/admin";

export const dynamic = "force-dynamic";
export const metadata = { title: "Testimonials" };

async function getTestimonials() {
  const sb = createAdminClient();
  const { data, error } = await sb
    .from("testimonials")
    .select(
      "id, client_name, company, role, quote, industry, is_featured, display_order, status, updated_at"
    )
    .order("display_order", { ascending: true });
  return { rows: data ?? [], error: error?.message };
}

const STATUS_STYLE: Record<string, string> = {
  published: "bg-success/10 text-success",
  draft: "bg-warm/10 text-warm",
  archived: "bg-slate-mute/20 text-slate",
};

export default async function AdminTestimonialsPage() {
  const { rows, error } = await getTestimonials();

  return (
    <main className="p-6 md:p-10">
      <div className="max-w-6xl">
        <div className="mb-8 flex flex-wrap items-end justify-between gap-3">
          <div>
            <p className="eyebrow-brand">Content</p>
            <h1 className="font-display mt-2 text-3xl md:text-4xl text-ink">
              Testimonials <span className="text-slate-mute tabular">({rows.length})</span>
            </h1>
            <p className="mt-2 text-sm text-slate">
              Featured testimonials muncul di homepage marquee + /clients page.
            </p>
          </div>
          <Link
            href="/admin/content/testimonials/new"
            className="inline-flex items-center justify-center rounded-full bg-brand text-paper px-5 h-10 text-sm font-medium hover:bg-brand-deep transition"
          >
            + New testimonial
          </Link>
        </div>

        {error && (
          <div className="mb-6 rounded-2xl border border-error/30 bg-error/5 p-4 text-sm text-error">
            DB error: <code className="text-xs">{error}</code>
          </div>
        )}

        {rows.length === 0 ? (
          <div className="rounded-2xl border border-border bg-paper p-12 text-center text-sm text-slate">
            Belum ada testimonial.
          </div>
        ) : (
          <div className="rounded-2xl border border-border bg-paper overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-xs uppercase tracking-wider text-slate-mute border-b border-divider bg-bone/50">
                  <th className="px-6 py-3 font-medium">Client</th>
                  <th className="px-4 py-3 font-medium">Company · Role</th>
                  <th className="px-4 py-3 font-medium">Industry</th>
                  <th className="px-4 py-3 font-medium">Featured</th>
                  <th className="px-4 py-3 font-medium">Status</th>
                  <th className="px-4 py-3 font-medium tabular text-right">Order</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((t) => (
                  <tr
                    key={t.id}
                    className="border-b border-divider/60 hover:bg-cream/40 transition"
                  >
                    <td className="px-6 py-4">
                      <Link
                        href={`/admin/content/testimonials/${t.id}`}
                        className="font-medium text-ink hover:text-brand-deep"
                      >
                        {t.client_name}
                      </Link>
                      <p className="mt-1 text-xs text-slate line-clamp-2 max-w-md">
                        &ldquo;{t.quote}&rdquo;
                      </p>
                    </td>
                    <td className="px-4 py-4 text-xs text-slate">
                      {t.company}
                      {t.role && <span className="text-slate-mute"> · {t.role}</span>}
                    </td>
                    <td className="px-4 py-4 text-xs text-slate">{t.industry ?? "—"}</td>
                    <td className="px-4 py-4">
                      {t.is_featured ? (
                        <span className="inline-flex items-center rounded-full bg-brand/10 text-brand-deep px-2 py-0.5 text-[11px] font-medium">
                          ★
                        </span>
                      ) : (
                        <span className="text-slate-mute text-xs">—</span>
                      )}
                    </td>
                    <td className="px-4 py-4">
                      <span
                        className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium ${
                          STATUS_STYLE[t.status] ?? "bg-cream text-slate"
                        }`}
                      >
                        {t.status}
                      </span>
                    </td>
                    <td className="px-4 py-4 text-right tabular text-xs text-slate">
                      {t.display_order}
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        )}
      </div>
    </main>
  );
}
