 ...
 * function checks `auth.uid()`'s email == 'superadmin@admin.local'
 * before doing anything, so this can't be abused by other users.
 *
 * Use case: handle_new_user trigger failed to fire OR Vercel's
 * SUPABASE_SERVICE_ROLE_KEY is misconfigured. /admin shows "Profile
 * not found" → click button → land on dashboard.
 */
...
async function handle() {
  ...
  return NextResponse.redirect(new URL("/admin", siteBase()));
}
