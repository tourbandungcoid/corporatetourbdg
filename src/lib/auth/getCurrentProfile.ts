 ...
 *
 * Uses the user-session server client (NOT the service-role admin
 * client) so we don't depend on SUPABASE_SERVICE_ROLE_KEY being
 * correctly configured. The RLS policy `profiles_select_self_or_admin`
 * allows a user to read their own profile via auth.uid().
 */
